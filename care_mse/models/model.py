...
class contrast_loss(nn.Module):
    ...
    def loss(self, anchor, positive):

        # L2 normalize
        anchor_norm, positive_norm = anchor.norm(), positive.norm()
        
        # mse
        loss = -2. * torch.mean(torch.sum(anchor * positive, dim=-1) / (anchor_norm * positive_norm))
