...
class sleep_pretrain(nn.Module):
    ...
    def __init__(self, config, name, dataloader, wandb_logger):
        ...
        TEST_FILE = [
            os.path.join(self.config.src_path, "test", f) for f in TEST_FILE
        ]
        self.test_subjects = [np.load(file) for file in TEST_FILE]

    def training_step(self, batch, batch_idx):
        weak, strong = batch
        weak, strong = weak.float().to(self.device), strong.float().to(
            self.device)
    ...
    def on_epoch_end(self):
        ...
        torch.save(full_chkpoint,
                   os.path.join(config.exp_path, self.name + "_full" + '.pt'))
        return None

    def ft_fun(self, epoch, train_dl, valid_dl):
        sleep_eval = sleep_ft(config.exp_path + "/" + self.name + '.pt',
                              self.config, train_dl, valid_dl, epoch,
                              self.loggr)
    ...
    def do_kfold(self):
        ...
        for train_idx, test_idx in kfold.split(self.test_subjects):
            ...
            train_loader = DataLoader(TuneDataset(test_subjects_train),
                                      batch_size=self.config.batch_size,
                                      shuffle=True)
            test_loader = DataLoader(TuneDataset(test_subjects_test),
                                     batch_size=self.config.batch_size,
                                     shuffle=False)

            f1, kappa, bal_acc, acc = self.ft_fun(self.name, train_loader,
                                                  test_loader)
            k_f1 += f1
            k_kappa += kappa
            k_bal_acc += bal_acc
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch % 5 == 0):
                ...
                if self.max_f1 < f1:
                    ...
                    torch.save(
                        chkpoint,
                        os.path.join(config.exp_path, self.name + f'_best.pt'))
                    self.loggr.save(
                        os.path.join(config.exp_path, self.name + f'_best.pt'))
                    self.max_f1 = f1
        ...
        for ep in tqdm(range(self.ft_epoch), desc='Evaluation'):
            ...
            with torch.no_grad():
                for ft_batch_idx, ft_batch in enumerate(self.valid_ft_dl):
                    ...
                    loss, acc, preds, target = dct['loss'], dct['acc'], dct[
                        'preds'], dct['target']
