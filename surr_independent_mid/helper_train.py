...
class sleep_pretrain(nn.Module):
    ...
    def ft_fun(self, file_name, epoch, test_subjects_train, test_subjects_test, split):

        train_dl = DataLoader(TuneDataset(test_subjects_train), batch_size=self.config.batch_size, shuffle=True)
        test_dl = DataLoader(TuneDataset(test_subjects_test), batch_size=self.config.batch_size, shuffle= False)
    ...
    def do_kfold(self):
        ...
        kfold = KFold(n_splits=self.config.splits, shuffle=True, random_state=1234)
        ...
        for train_idx, test_idx in kfold.split(self.test_subjects):
            ...
            print(f"Split {i}")
            f1, kappa, bal_acc, acc = self.ft_fun(
                self.name, 0, test_subjects_train, test_subjects_test, i
            )
            k_f1 += f1
            k_kappa += kappa
            k_bal_acc += bal_acc
        ...
        return k_f1 / self.config.splits, k_kappa / self.config.splits, k_bal_acc / self.config.splits, k_acc / self.config.splits
